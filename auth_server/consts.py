...
DB_CONFIG = {
    ...
    MESSAGE_SERVERS_TABLE: {
        "create_command": f"CREATE TABLE IF NOT EXISTS {MESSAGE_SERVERS_TABLE} (ID BLOB({ID_SIZE}) PRIMARY KEY, name TEXT({NAME_SIZE}) NOT NULL, IP BLOB({MESSAGE_SERVERS_IP_SIZE}) NOT NULL, port INTEGER({MESSAGE_SERVERS_PORT_SIZE}) NOT NULL, AESKey BLOB({CLIENTS_AES_KEY_SIZE}) NULL)",
        "get_servers": f"SELECT * FROM {MESSAGE_SERVERS_TABLE}",
        "add_server": f"INSERT INTO {MESSAGE_SERVERS_TABLE} (ID, name, IP, port, AESKey) VALUES (:id, :name, :ip, :port, :aes_key)",
        "data_class": "Server"
    }
}
