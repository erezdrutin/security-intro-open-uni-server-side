...
class ProtocolHandler(BaseProtocol):
    ...
    def _handle_client_authentication(self, client_socket: socket.socket,
        ...
        print(f"CLIENT REQUEST LENGTH: {len(request.payload)}")
        ...
        #     data=ticket_bytes, cipher=server_cipher)

        print(ticket)
        # Extract authenticator from request:
        ...
        # 1. Test if authenticator.creation_time < ticket.expiration_time
        # 2. Test if authenticator.client_id == ticket.client_id
        # 3. Test if authenticator.server_id == self.server_id
        # 4. Test if ticket.version == authenticator.version
        # if all are true --> Return Valid!
        # otherwise --> return 1609 GENERAL SERVER ERROR
        authenticator.creation_time

        print("ok cool. what now?")
